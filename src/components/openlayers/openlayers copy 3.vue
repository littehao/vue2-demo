...
export default {
  ...
  methods: {
    initMap() {
      ...
      var coordinate = olProj.transform(this.center, "EPSG:4326", "EPSG:3857");
      console.log(
        coordinate,
        this.convertToBaidu(this.center[0], this.center[1])
      );
      new Map({
        target: "map",
        layers: [baidu_layer],
        view: new View({
          projection: "EPSG:3857",
          // center: this.convertToBaidu(this.center[0], this.center[1]),
          center: coordinate,
          // center: this.transformFromWGS84ToBD09(this.center),
          zoom: 12,
        }),
      });
    },
